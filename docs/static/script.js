...
async function loadConstitutionData() {
    showLoading();
    try {
        const response = await fetch('constitution_data.json');
...
function generateLegalAdvice(query, relevantArticles) {
    ...
    if (/\blanguage|hindi|english|official\b/.test(queryLower)) {
        adviceParts.push("\n**Official Language:**");
        adviceParts.push("Article 343 declares Hindi in Devanagari script as the official language of the Union, with English continued for official purposes.");
    }
